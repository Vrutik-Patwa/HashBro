  ...
  return (
    <article className={cn("group flex flex-col rounded-2xl border border-border bg-white overflow-hidden shadow-sm transition-all hover:shadow-lg hover:-translate-y-1", className)}>
      <Link href={`/shop/${product.slug}`} className="relative aspect-square overflow-hidden bg-muted">
        <Image src={product.image} alt={product.name} fill className="object-cover transition-transform group-hover:scale-105" sizes="(max-width:640px) 100vw, 33vw" />
        {product.bestseller && (
          <span className="absolute top-3 left-3 rounded-full bg-hasbro-yellow px-3 py-1 text-xs font-bold">Bestseller</span>
        )}
      ...
      <div className="flex flex-1 flex-col p-5">
        <span className="text-xs font-semibold text-hasbro-red uppercase">{product.category}</span>
        <Link href={`/shop/${product.slug}`}>
          <h3 className="mt-1 text-lg font-bold text-hasbro-charcoal line-clamp-2 group-hover:text-hasbro-red">{product.name}</h3>
        </Link>
